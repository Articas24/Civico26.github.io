(()=>{
'use strict';
if(window.__civicoAdminFinalFixesV2)return;window.__civicoAdminFinalFixesV2=true;
const q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)];
let queued=false;

function ensureStyles(){
 if(q('#civicoFinalFixStylesV2'))return;
 document.head.insertAdjacentHTML('beforeend',`<style id="civicoFinalFixStylesV2">
...
function reorderTabs(){
 ...
 const insight=byId.get('aiInsightsSec');
 if(insight&&insight.textContent.trim()!=='AI Insight')insight.textContent='AI Insight';
 ...
 const already=target.length===all.length&&target.every((node,i)=>all[i]===node);
 if(already)return;
 target.forEach(node=>tabs.appendChild(node));
}
...
function periodLabel(){
 ...
 const y=q('#historyStatsYear')?.value||new Date().getFullYear();
 if(mode==='month'){
  const m=q('#svMonth');return `${m?.selectedOptions?.[0]?.textContent||''} ${y}`.trim();
 }
 if(mode==='custom'){
...
function boot(){
 patchAll();
 const root=q('#adminView')||document.body;
