...
@media(max-width:560px){#statsSec .sv-channel{font-size:12px!important;padding:6px 0}#statsSec .sv-channel span{font-size:12px!important}#statsSec .sv-channel b{font-size:11px!important}#statsSec .sv-donut{width:152px!important;height:152px!important}#statsSec .sv-donut-pct{font-size:9px;padding:2px 5px}#statsSec .sv-breakdown{grid-template-columns:1fr}.ota-brand-icon{width:16px;height:16px}}
</style>`)}
...
function enhancePlatforms(){qa('#statsSec .sv-platform').forEach(card=>{const h=card.querySelector('h3');if(!h||h.querySelector('.ota-brand-icon'))return;const p=sourceFromText(h.textContent);if(p)h.insertAdjacentHTML('afterbegin',icon(p))})}
function enhanceChannels(){const rows=qa('#statsSec .sv-channel');rows.forEach(row=>{const name=row.querySelector('span');if(!name)return;name.classList.add('sv-channel-name');if(name.querySelector('.ota-brand-icon'))return;const p=sourceFromText(name.textContent);if(p)name.insertAdjacentHTML('afterbegin',icon(p))});const donut=q('#statsSec .sv-donut');if(!donut)return;qa('.sv-donut-pct',donut).forEach(x=>x.remove());let cursor=0;rows.forEach(row=>{const m=(row.querySelector('b')?.textContent||'').match(/(\d+(?:[.,]\d+)?)%/);if(!m)return;const p=Number(m[1].replace(',','.'));if(!p)return;const mid=cursor+p/2,rad=(mid/100*360-90)*Math.PI/180,r=window.innerWidth<=560?50:59,x=Math.cos(rad)*r,y=Math.sin(rad)*r,label=document.createElement('span');label.className='sv-donut-pct';label.textContent=`${Math.round(p)}%`;label.style.marginLeft=`${x}px`;label.style.marginTop=`${y}px`;donut.appendChild(label);cursor+=p})}
...
function platform(name){const card=qa('#statsSec .sv-platform').find(x=>(x.querySelector('h3')?.textContent||'').trim().toLowerCase().includes(name));if(!card)return null;const get=label=>{const row=qa('.sv-row',card).find(x=>(x.querySelector('span')?.textContent||'').trim().toLowerCase()===label);return moneyText(row?.querySelector('b')?.textContent||'')};const gross=get('lordo report')??0,commission=get('commissioni')??0,vat=get('iva servizi')??0,fee=get('costi transazione')??0,cost=get('costi piattaforma totali')??(commission+vat+fee),tax=get('cedolare secca')??0,payout=get('netto accreditato');return{gross,commission,vat,fee,cost,tax,payout,net:gross-cost-tax}}
...
function breakdown(kind){const b=platform('booking'),a=platform('airbnb');if(!b&&!a)return '';const map={gross:['Lordo','gross'],net:['Netto finale','net'],cost:['Costi piattaforma','cost'],tax:['Cedolare secca','tax']},[label,key]=map[kind]||map.gross;const one=(p,name)=>`<div><span>${icon(name.toLowerCase())}${label} ${name}</span><b>${fmt(p?.[key]??0)}</b></div>`;return one(b,'Booking')+one(a,'Airbnb')}
...
function enhanceClickableCards(){const map={'Lordo piattaforme':'gross','Netto finale':'net','Costi piattaforme':'cost','Cedolare secca':'tax'};qa('#statsSec .sv-econ > .sv-kpi').forEach(card=>{const title=(card.querySelector('span')?.textContent||'').trim(),kind=map[title];if(!kind||card.dataset.clickReady)return;card.dataset.clickReady='1';card.dataset.kind=kind;card.classList.add('sv-clickable-kpi');card.setAttribute('role','button');card.setAttribute('tabindex','0');card.addEventListener('click',()=>toggleBreakdown(kind));card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggleBreakdown(kind)}})})}
function run(){styles();enhanceFuture();enhanceFeeds();enhancePlatforms();enhanceChannels();enhanceClickableCards()}
function boot(){run();const root=q('#adminView')||document.body;let scheduled=false;new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;run()})}).observe(root,{childList:true,subtree:true});document.addEventListener('click',e=>{if(e.target.closest('.navtab[data-section="futureSec"],.navtab[data-section="syncSec"],.navtab[data-section="statsSec"]'))setTimeout(run,120)});window.addEventListener('resize',()=>{if(q('#statsSec .sv-donut'))run()})}
boot();
})();