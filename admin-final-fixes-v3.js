(()=>{
'use strict';
if(window.__civicoAdminFinalFixesV3)return;window.__civicoAdminFinalFixesV3=true;
const q=s=>document.querySelector(s),qa=s=>[...document.querySelectorAll(s)];
let queued=false;

function ensureStyles(){
 if(q('#civicoFinalFixStylesV3'))return;
 document.head.insertAdjacentHTML('beforeend',`<style id="civicoFinalFixStylesV3">
/* Ordine visivo stabile della barra, indipendente dall'ordine di creazione JS */
.navtabs .navtab{order:100!important}
.navtabs .navtab[data-section="calendarSec"]{order:10!important}
.navtabs .navtab[data-section="pricesSec"]{order:20!important}
.navtabs .navtab[data-section="statsSec"]{order:30!important}
.navtabs .navtab[data-section="aiSec"]{order:40!important}
.navtabs .navtab[data-section="aiInsightsSec"]{order:50!important}
.navtabs .navtab[data-section="reportImportSec"]{order:60!important}
.navtabs .navtab[data-section="futureSec"]{order:70!important}
.navtabs .navtab[data-section="requestsSec"]{order:80!important}
.navtabs .navtab[data-section="syncSec"]{order:90!important}
...
#aiInsightsSec .ip-head{background:#f2f7f5!important;border:1px solid var(--line)!important;color:var(--muted);font-size:10.5px!important;font-weight:900!important;text-transform:uppercase;letter-spacing:.035em}
...
@media(min-width:901px){
 #aiSec .ai-main{overflow:visible!important;max-height:none!important;height:auto!important}
 #aiSec .ai-chat{max-height:none!important;height:auto!important;overflow:visible!important;min-height:430px}
 #aiSec .ai-msg,#aiSec .ai-msg.assistant{max-height:none!important;overflow:visible!important;white-space:normal!important}
}
...
function safeInsightTabLabel(){
 const tab=q('.navtab[data-section="aiInsightsSec"]');if(!tab)return;
 let badge=tab.querySelector('#ibadge');
 if(!badge){badge=document.createElement('span');badge.id='ibadge';tab.appendChild(badge)}
 let text=[...tab.childNodes].find(n=>n.nodeType===Node.TEXT_NODE);
 if(!text){text=document.createTextNode('AI Insight');tab.insertBefore(text,badge)}
 else if(text.nodeValue!=='AI Insight')text.nodeValue='AI Insight';
}
...
function patchInsights(){
 ...
 if(sim){
  ...
  Object.entries(labels).forEach(([id,label])=>{
   const el=q('#'+id);if(!el||el.closest('.sim-field')||!el.parentNode)return;
   const wrap=document.createElement('label');wrap.className='sim-field';wrap.htmlFor=id;
...
function parseMoney(text){
 const s=String(text||'').replace(/\s/g,'').replace(/[^0-9,.-]/g,'');if(!s)return null;
...
function periodLabel(){
 ...
 if(mode==='month'){const m=q('#svMonth');return `${m?.selectedOptions?.[0]?.textContent||''} ${y}`.trim()}
 if(mode==='custom'){const a=q('#svStart')?.value,b=q('#svEnd')?.value;return a&&b?`${a} → ${b}`:'periodo personalizzato'}
 return `anno ${y}`;
}
function patchStatsCoverage(){
 const box=q('#statsSec .sa-coverage');if(!box)return;
 const p=periodLabel(),muted=box.querySelector('.sa-coverage-head .muted'),h=box.querySelector('.sa-coverage-head h3');
 if(muted)muted.textContent=`Copertura economica · ${p}`;
 if(h)h.textContent=`Consuntivo vs prenotato · ${p}`;
 const status=box.querySelector('.sa-status.partial');if(status)status.textContent='Periodo parziale';
 const grid=box.querySelector('.sa-grid');if(!grid)return;
 let net=grid.querySelector('.sa-net-indicative');
 if(!net){net=document.createElement('div');net.className='sa-box sa-net-indicative';net.innerHTML='<span>Netto indicativo</span><strong>—</strong><small class="sa-net-method">stima sul periodo selezionato</small>';grid.appendChild(net)}
 ...
 let ratio=actualGross&&actualGross>0&&actualNet!=null?actualNet/actualGross:null;if(ratio!=null&&(ratio<=0||ratio>1.05))ratio=null;
 ...
 const note=box.querySelector('.sa-note');if(note&&!note.dataset.netExplained){note.insertAdjacentText('beforeend',' Il netto indicativo applica al lordo indicativo il rapporto netto/lordo già consuntivato nel periodo selezionato.');note.dataset.netExplained='1'}
}

function patchAll(){queued=false;try{ensureStyles();safeInsightTabLabel();patchInsights();patchStatsCoverage()}catch(e){console.error('Admin final fixes v3:',e)}}
function schedule(){if(queued)return;queued=true;requestAnimationFrame(patchAll)}
function boot(){
 patchAll();
 new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true});
 document.addEventListener('change',e=>{if(e.target?.matches?.('#svMode,#historyStatsYear,#svMonth,#svStart,#svEnd'))setTimeout(patchStatsCoverage,80)});
}
