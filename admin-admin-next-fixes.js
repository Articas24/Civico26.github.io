...
function compareRanges(){const mode=q('#svCmpMode')?.value||'same';if(mode==='same'){const a=currentRange();return periodPolicy?.sameRanges(a,(q('#svMode')?.value||'year')==='year')||[a,shiftYear(a,-1)]}if(mode==='year'){const a=Number(q('#svCmpYearA')?.value),b=Number(q('#svCmpYearB')?.value);if(Number.isFinite(a)&&Number.isFinite(b))return periodPolicy?.yearRanges(a,b)||[{start:new Date(a,0,1),end:new Date(a+1,0,1)},{start:new Date(b,0,1),end:new Date(b+1,0,1)}]}if(mode==='month'){const a=Number(q('#svCmpYearA')?.value),b=Number(q('#svCmpYearB')?.value),m=Number(q('#svCmpMonth')?.value)||1;if(Number.isFinite(a)&&Number.isFinite(b))return periodPolicy?.monthRanges(a,b,m)||[{start:new Date(a,m-1,1),end:new Date(a,m,1)},{start:new Date(b,m-1,1),end:new Date(b,m,1)}]}const sa=q('#svCmpStartA')?.value,ea=q('#svCmpEndA')?.value,sb=q('#svCmpStartB')?.value,eb=q('#svCmpEndB')?.value;if(sa&&ea&&sb&&eb)return[{start:parse(sa),end:plus(parse(ea),1)},{start:parse(sb),end:plus(parse(eb),1)}];return null}
...
function axisData(ra,rb){
  const ma=periodPolicy?.monthBuckets(ra,months).map(x=>x.label)||[],mb=periodPolicy?.monthBuckets(rb,months).map(x=>x.label)||[];
  if(ma.length&&ma.length===mb.length&&ma.every((x,i)=>x===mb[i]))return{kind:'months',rows:[{labels:ma}],help:'Ogni punto rappresenta lo stesso mese nei due anni; i mesi non ancora trascorsi sono esclusi.'};
